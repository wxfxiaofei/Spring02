...
public class Main {
    public static void main(String[] args) {
        ...
        //面向切面编程的术语
        //1.切面(Aspect):一个关注点的模块化,这个关注点(可能会横切多个对象,通常是一个类
        //2.连接点(JoinPoint):程序执行的某个位置(连接点是切面插入应用程序的地方)
        //3.通知(Advice):切面在连接点处执行的动作
        //4.切入点(Pointcut):切面选择的连接点
        //5.目标对象(TargetObject):被切面通知的对象
        //6.织入(Weaving):把切面跟对象关联,并创建该对象的代理对象的过程,(简单地说,就是指将切面代码插入到目标对象的过程)
        //7.引入(Introduction):在不修改代码的前提下,引入可以在运行期为类状态添加一些方法或字段
